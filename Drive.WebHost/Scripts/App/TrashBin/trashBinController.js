(function () {
    ...
    function TrashBinController(trashBinService, fileService, $routeParams, toastr, localStorageService) {
        ...
        function search() {
            trashBinService.searchTrashBin(vm.searchText,
                function(data) {
                    vm.spaces = data;
                    console.log(data);
                });
        ...
        function restoreFile(id, spaceId) {
            trashBinService.restoreFile(id,
                function() {
        ...
        function restoreFolder(id, spaceId) {
            trashBinService.restoreFolder(id,
                function() {
                    deleteFolderFromScope(id, spaceId);
                    toastr.success(
                        'Folder was successfully restored!',
                        'Trash bin',
                        {
                            closeButton: true,
                            timeOut: 5000
        ...
        function restoreSpace(space, index) {
            ...
            trashBinService.restoreSpaces(spaces,
                function() {
                    vm.spaces.splice(index, 1);
                    toastr.success(
                        'All content of ' + space.name + ' successfully restored!',
                        'Trash bin',
                        {
                            closeButton: true,
                            timeOut: 5000
        ...
        function restoreTrashBin() {
            ...
            trashBinService.restoreSpaces(spaces,
                function() {
                    vm.spaces = [];
                    toastr.success(
                        'All contents of the trash successfully restored',
                        'Trash bin',
                        {
                            closeButton: true,
                            timeOut: 5000
        ...
        function deleteFilePermanently(id, spaceId) {
            swal({
                ...
                function() {
                    trashBinService.deleteFilePermanently(id,
                        function() {
                            deleteFileFromScope(id, spaceId);
                        });
                    swal({
        ...
        function deleteFolderPermanently(id, spaceId) {
            swal({
                ...
                function() {
                    trashBinService.deleteFolderPermanently(id,
                        function() {
                            deleteFolderFromScope(id, spaceId);
                        });
                    swal({
        ...
        function clearSpace(space, index) {
            swal({
                ...
                function() {
                    trashBinService.clearSpace(space.spaceId,
                        function() {
                            vm.spaces.splice(index, 1);
                        });
                    swal({
        ...
        vm.spaceMenuOptions = [
            [
                'Restore all for this space', function($itemScope) {
                    vm.restoreSpace($itemScope.space, $itemScope.$index);
                }
            ],
